...
options.output = "cmgTuple_newId_vbf.root" 
...
##GIULIO
process.htcut = cms.EDFilter("PartonLevelHTFilter",
    src = cms.InputTag("source"),
    htlessthan = cms.double(200)
)
...
process.GlobalTag.globaltag = 'START53_V22::All' #getGlobalTag( runOnMC, runOld5XGT )
print 'Global tag       : ', process.GlobalTag.globaltag



###Simple memory check. Look for memory leaks#####################################
...
for content in options.content:
    ...
    if content == "ele":
        ...
        process.out.outputCommands.append( 'keep *_ZeeCand*_*_CMG')
        process.out.outputCommands.append( 'keep *_LDe_*_CMG')
    if content == "mu":
        ...
        process.out.outputCommands.append( 'keep *_ZmmCand*_*_CMG')
        process.out.outputCommands.append( 'keep *_LDmu_*_CMG')
    if content == "gen":
...
##         process.out.outputCommands.append( 'keep *_patMETs*_*_*' )
        ...
        process.out.outputCommands.append( 'keep *_ZjjCand*_*_CMG')
...
process.cmgElectron2L2Q.cuts.cutbasedloose = cms.string('sourcePtr().electronID(\"eidLoose\")')

process.electronPreselNoIso.cut = cms.string("getSelection(\"cuts_kinematics\") "
                                             #+"&& getSelection(\"cuts_loosemvaid\") "
                                             +"&& getSelection(\"cuts_cutbasedloose\") "
...
process.muonPreselIsoDeltaB = process.muonPreselNoIso.clone()
process.muonPreselIsoDeltaB.cut.IsoDeltaBeta = cms.PSet( IsoDeltaBeta = cms.string('(sourcePtr().get().pfIsolationR04().sumChargedHadronPt+max(0.,(sourcePtr().get().pfIsolationR04().sumPhotonEt+sourcePtr().get().pfIsolationR04().sumNeutralHadronEt-0.5*sourcePtr().get().pfIsolationR04().sumPUPt)))/sourcePtr().get().pt()<0.12') )
process.cmgDiMuon2L2Q.cfg.leg1Collection = cms.InputTag("muonPreselIsoDeltaB")
process.cmgDiMuon2L2Q.cfg.leg2Collection = cms.InputTag("muonPreselIsoDeltaB")
#process.cmgMuon2L2Q.
...
process.cmgJetRaw.cuts.jetKinematics.pt = 'pt()>20.'
...
#remove the kin fit related processes
process.higgsSequenceEle.remove(process.cmgDiElectronDiJetKinFitEle)
process.higgsSequenceEle.remove(process.cmgDiElectronDiJetKinFitHiggsEle)
process.higgsSequenceEle.remove(process.HLTWeightsKinFitEle)
process.higgsSequenceEle.remove(process.cmgHiggsKinFitWeighted2012AEle)
process.higgsSequenceEle.remove(process.cmgHiggsKinFitWeighted2012BEle)
process.higgsSequenceEle.remove(process.cmgHiggsKinFitWeightedEle)
process.higgsSequenceEle.remove(process.cmgHiggsSelKinFitEle)

print process.higgsSequenceMu

process.higgsSequenceMu.remove(process.cmgDiMuonDiJetKinFitMu)
process.higgsSequenceMu.remove(process.cmgDiMuonDiJetKinFitHiggsMu)
process.higgsSequenceMu.remove(process.HLTWeightsKinFitMu)
process.higgsSequenceMu.remove(process.cmgHiggsKinFitWeighted2012AMu)
process.higgsSequenceMu.remove(process.cmgHiggsKinFitWeighted2012BMu)
process.higgsSequenceMu.remove(process.cmgHiggsKinFitWeightedMu)
process.higgsSequenceMu.remove(process.cmgHiggsSelKinFitMu)


process.LDe=cms.EDProducer("DiElectronDiJetHiggsLDProducer",
                      src       =cms.InputTag("cmgHiggsSelEle"),
...
process.LDmu=cms.EDProducer("DiMuonDiJetHiggsLDProducer",
                      src       =cms.InputTag("cmgHiggsSelMu"),
                      spin      =cms.untracked.int32(0),
                      paramfile = cms.FileInPath("Alessio/RooFitUtil/src/PDFs/allSignalParams.txt"),
                      nhanfile = cms.FileInPath("Alessio/RooFitUtil/src/PDFs/fqqTable.txt"),
                      )
print process.higgsSequenceEle
print process.higgsSequenceMu
...
process.preselEle = cms.Path(process.htcut+process.badEventFilters+process.analysisSequenceHZZEE)
process.preselMu = cms.Path(process.htcut+process.badEventFilters+process.analysisSequenceHZZMM)
...
process.schedule.append(process.OutPath)
#append the memory check
#process.schedule.append(process.SimpleMemoryCheck)
##############
## Low mass ##
##############
...
process.zmumu.leg1_kinematics.pt = cms.string('(leg1().pt() > 20 && leg2().pt() > 10)')
#||(leg1().pt() > 10 && leg2().pt() > 5)')

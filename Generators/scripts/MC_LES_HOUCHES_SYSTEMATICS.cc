...
namespace Rivet {
  ...
  public:
    ...
    void analyze(const Event & event) {
      ...
      if (jets.size() >= 1) {
        _h_dEtaj0l->fill(deltaEta(jets[0],lepton));
        _h_dPhij0l->fill(deltaPhi(jets[0],lepton));
        _h_dRj0l->fill(deltaR(jets[0],lepton));
        _h_mj0l->fill((jets[0].momentum() + lepton.momentum()).mass());
      }

      if (jets.size() >= 2) {
        _h_dEtaj0j1->fill(deltaEta(jets[0],jets[1]));
        _h_dPhij0j1->fill(deltaPhi(jets[0],jets[1]));
        _h_dRj0j1->fill(deltaR(jets[0],jets[1]));
        _h_mj0j1->fill((jets[0].momentum() + jets[1].momentum()).mass());
        _h_mj0j1W->fill((jets[0].momentum() + jets[1].momentum() + Wmom).mass());
        _h_ptratioj1j0->fill(jets[1].momentum().pT()/jets[0].momentum().pT());
      }
